...
// finalized black & white design, address positioned for a standard
// #10 window envelope. Requires jsPDF loaded on window (see layout.js).
//
// The banner/tag/details-box/signature/QR/footer pieces are exported so
// lib/partnerLetters.js (realtor/broker, property manager, title company
// outreach) can reuse the same visual system with different copy.

export const PAGE_W = 215.9, PAGE_H = 279.4; // mm, US Letter
...
const LEVELS = {
  1: {
    ...
    tagStyle: 'outline',
    intent:
      'Our records show that a permit tied to your property has expired without a final ' +
      'inspection or closed status. This is the first notice we are sending to make you ' +
      'aware of the situation while it is still simple to resolve.',
    urgency:
      'Acting now, while the fix is straightforward, is always easier than waiting until ' +
      'it affects a sale, refinance, or insurance renewal.',
    urgencyBold: false,
  },
  2: {
    ...
    tagStyle: 'gray',
    intent:
      'This is our second notice regarding the expired permit on your property. Our records ' +
      'still show no final inspection or closure on file. The longer an expired permit sits ' +
      'open, the more it can complicate a future sale, refinance, or insurance claim.',
    urgency:
      'We recommend addressing this before it becomes a larger, more costly issue at closing ' +
      'or renewal time.',
    urgencyBold: true,
  },
  3: {
    ...
    tagStyle: 'solid',
    intent:
      'This is our final notice. The permit referenced below remains expired and unresolved ' +
      'on our records. Unresolved permits are a matter of public record and can surface ' +
      'unexpectedly during a title search, sale, refinance, or insurance inspection.',
    urgency:
      'This is your last courtesy notice from our office. After this, we will assume you have ' +
      'chosen to handle the matter independently or through another party.',
    urgencyBold: true,
  },
};
...
// Draws a scannable QR code as vector squares (not a rasterized image) so it
// stays crisp at print resolution. Requires qrcode-generator loaded on window.
export function drawQRCode(doc, text, x, y, size) {
  ...
  const quiet = 2; // modules of white margin baked into the box, per QR spec
...
// Black banner with seal + wordmark on the left, an escalation tag on the right.
// Returns the banner height so callers know where content starts.
export function drawLetterBanner(doc, { eyebrow, tag, tagStyle, subhead = 'Independent Permit Resolution Service', wordmark = 'THE PERMIT CLOSER' }) {
  const W = PAGE_W;
...
// Grey-filled details box with a title, an optional right-aligned badge, and
// up to 4 short label/value fields laid out in even columns.
...
// Signature block (left) + "scan to learn more" QR (right) on the same
// vertical band, followed by the black contact footer. Call once content is done.
export function drawSignatureAndFooter(doc, y, opts = {}) {
  const {
    ...
    qrUrl = 'https://thepermitcloser.com',
  } = opts;
...
// Draws a bulleted "how we can help" list starting at y, returns the new y.
...
export function drawNoticeLetter(doc, level, data) {
  ...
  drawLetterBanner(doc, { eyebrow: L.eyebrow, tag: L.tag, tagStyle: L.tagStyle });
  ...
  y = drawDetailsBox(doc, y, 'PERMIT NOTICE DETAILS', 'STATUS: EXPIRED', [
  ...
  y = drawHelpPoints(doc, y, 'How The Permit Closer Can Help', HELP_POINTS);

  drawSignatureAndFooter(doc, y);
}

// Splits a lead's full address into two lines for the letter (street / city-state-zip).
